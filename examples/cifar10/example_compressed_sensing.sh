...
# seq 0 2 | parallel -k --lb -j8 CUDA_VISIBLE_DEVICES='$(({%} % 8))' \
...
# Step 2) Launch 50 workers, each handling row i in S
seq 0 999 | parallel -k --lb -j8 CUDA_VISIBLE_DEVICES='$(({%} % 8))' \
    python -m datamodels.training.worker \
        --worker.index={} \
        --worker.store_dir=log_dir \
